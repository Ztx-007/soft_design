from decimal import Decimal
from sqlalchemy.orm import Session
from model import Conversion, ConversionRate
from datetime import datetime
from exceptions import ExchangeRateNotFound
import logging

logger = logging.getLogger("backend.service")

class ExchangeRateService:
    def __init__(self, db: Session) -> None:
        self.db = db

    def convert(self, from_currency: str, to_currency: str, amount: Decimal) -> dict:
        from_currency = from_currency.upper()
        to_currency = to_currency.upper()

        rate_entry = (
            self.db.query(ConversionRate)
            .filter_by(from_currency=from_currency, to_currency=to_currency)
            .order_by(ConversionRate.timestamp.desc())
            .first()
        )

        if not rate_entry or rate_entry.rate <= 0:
            raise ExchangeRateNotFound(from_currency, to_currency)

        logger.info(f"Using rate{rate_entry.rate}")
        result = amount * rate_entry.rate

        conversion = Conversion(
            from_currency=from_currency,
            to_currency=to_currency,
            amount=amount,
            result=result,
            timestamp=datetime.now(),
        )

        self.db.add(conversion)
        self.db.commit()
        return {"result": result, "rate": rate_entry.rate}
